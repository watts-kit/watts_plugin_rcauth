...
credentials in seconds. defaults to proxyCertMaxLifetime attribute value
    ...
    def get(self,
            ...
            voName=None,
            vomsName=None):
        """ get a proxy with (potentially) VOMS parameters
        
        :type username: string
        :param username: username selected for the credential
        :type passphrase: string
        :param passphrase: pass-phrase for the credential.  This will be used \
by the server to authenticate the request.  It must be at least \
6 characters.  The server may impose other restrictions too depending \
on its configuration.
        :type sslCertFile: string
        :param sslCertFile: certificate used for client authentication with \
...
credentials in seconds. defaults to proxyCertMaxLifetime attribute value
        :type voName: string
        :param voName: VO name to connect VOMS server to
        :type vomsName: string
        :param vomsName: VOMS server name 
        """
...
If creds already, exist exit without proceeding
        ...
        # key and cert are now passed as params, not files
        certTxt = certFile
        keyTxt = keyFile
